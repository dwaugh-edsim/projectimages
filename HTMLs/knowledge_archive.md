# Classroom Sim Architect: Knowledge Archive (v65.1 - synced with codebase)
...
Provide a concise assessment (150 words). Format with markdown.`;
        ...
        function tab(el, tName) {
            ...
            else if (type === "video") h = `<iframe src="${url}" allowfullscreen style="width:100%;height:100%;border:none;"></iframe>`;
            ...
            // FAILSAFE RENDERER v63.1
            let rawTxt = c.body || c.content || "";
            let finalHtml = parseMD(rawTxt);
            if (!finalHtml && rawTxt) finalHtml = `<div style="white-space:pre-wrap; font-family:var(--font-p);">${rawTxt}</div>`; // Fallback if MD fails
