...
function compareMatches(student, correct) {
    ...
    const normalize = (str) => {
        return str
            ...
            .replace(/\b(professor|prof|mr|mr\.?)\s+smith\b/gi, 'mr waugh')  // Accept teacher name variations
            .trim();
    };
...
// Grade definition quality
function gradeDefinition(term, definition) {
    if (!definition || definition.trim() === '') {
        return { score: 0, maxScore: 3, feedback: 'No definition provided' };
    }

    const defLower = definition.toLowerCase();
    const keywords = DEFINITION_KEY[term] || [];
    let matchCount = 0;

    for (const keyword of keywords) {
        if (defLower.includes(keyword.toLowerCase())) {
            matchCount++;
        }
    }

    const score = Math.min(3, Math.floor((matchCount / keywords.length) * 4));
    const quality = score >= 3 ? 'good' : (score >= 2 ? 'partial' : 'weak');
    ...
    return {
        ...
        feedback: score >= 3 ? 'Good definition with key concepts' : (score >= 2 ? 'Partial understanding' : 'Needs more detail')
    };
}
...
function gradeStudent(student) {
    ...
    for (const [term, correctMatch] of Object.entries(CORRECT_MATCHES)) {
        const response = student.responses[term] || { definition: '', match: '' };
        const studentMatch = response.match || '';
        const definition = response.definition || '';

        const isCorrect = compareMatches(studentMatch, correctMatch);
        const isEmpty = !studentMatch || studentMatch.trim() === '';
        ...
        // Grade definition
        const defGrade = gradeDefinition(term, definition);
        definitions.push({
            term: term,
            definition: definition,
            ...defGrade
    ...
    const status = correctCount === 15 ? 'complete' : (correctCount >= 10 ? 'partial' : 'incomplete');
    ...
    return {
        ...
        definitionCount: definitions.filter(d => d.definition.trim() !== '').length
    };
}

// Generate HTML results
function generateHTMLResults(studentsData) {
    const now = new Date();
...
<head>
    ...
    <meta name="last-updated" content="${isoTimestamp}">
    <style>
        :root {
            --primary: #6366f1;
            --primary-dark: #4f46e5;
...
<body>
    ...
    <script>
        const students = ${JSON.stringify(studentsData)};
...
function generateMarkdownSummary(studentsData) {
    const date = new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' });

    let md = `# Student Marking Results - March 30, ${new Date().getFullYear()}\n\n`;
    md += `**Generated:** ${date}\n\n`;
    md += `---\n\n`;

    // Summary stats
    const total = studentsData.length;
    const complete = studentsData.filter(s => s.status === 'complete').length;
    const partial = studentsData.filter(s => s.status === 'partial').length;
    const incomplete = studentsData.filter(s => s.status === 'incomplete').length;

    md += `## Summary\n\n`;
    md += `| Metric | Count |\n|--------|-------|\n`;
    md += `| Total Students | ${total} |\n`;
    md += `| Complete (15/15) | ${complete} |\n`;
    md += `| Partial (10-14/15) | ${partial} |\n`;
    md += `| Incomplete (<10/15) | ${incomplete} |\n\n`;
    ...
    for (const student of studentsData) {
        md += `| ${student.name} | ${student.pin} | ${student.correctMatches}/15 | ${student.status} | ${student.definitionCount}/15 |\n`;
    }
...
// Main function
function main() {
    const csvPath = path.join(__dirname, '..', 'adam debrief  mAR 30 3pm- Debriefs.csv');
    const outputPath = path.join(__dirname, '..', 'answerresults.html');
    const mdPath = path.join(__dirname, '..', 'markingresults3-30.md');

    console.log('Processing March 30 CSV data...');
    console.log(`Reading: ${csvPath}`);

    const csvContent = fs.readFileSync(csvPath, 'utf-8');
    const students = parseCSV(csvContent);

    console.log(`Found ${Object.keys(students)} students`);

    const gradedStudents = Object.values(students).map(gradeStudent);

    // Generate HTML
    const html = generateHTMLResults(gradedStudents);
    fs.writeFileSync(outputPath, html, 'utf-8');
    console.log(`Generated: ${outputPath}`);

    // Generate Markdown
    const md = generateMarkdownSummary(gradedStudents);
    fs.writeFileSync(mdPath, md, 'utf-8');
    console.log(`Generated: ${mdPath}`);

    // Print summary
    const complete = gradedStudents.filter(s => s.status === 'complete').length;
    const partial = gradedStudents.filter(s => s.status === 'partial').length;
    const incomplete = gradedStudents.filter(s => s.status === 'incomplete').length;

    console.log('\n=== Summary ===');
    console.log(`Total: ${gradedStudents.length}`);
