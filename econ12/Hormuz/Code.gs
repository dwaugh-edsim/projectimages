...
function doPost(e) {
  try {
    ...
    if (action === 'login') {
      ...
      if (rowIndex !== -1) {
        ...
        if (!savedName || savedName === "" || savedName === "undefined") {
          ...
          // Verify Identity: Does the provided name roughly match the saved name?
          if (savedName.toLowerCase().trim() !== name.toLowerCase().trim()) {
            throw new Error(`PIN in use. This PIN belongs to Advisor '${savedName}'. If this is not you, please choose a different 4-digit PIN.`);
          }
        }
    ...
    // Calls OpenRouter from the backend so the API Key is never exposed to the browser.
      ...
      if (!OPENROUTER_KEY) throw new Error("API Key configuration missing on server.");
      
      const model = payload.model || "qwen/qwen3-235b-a22b-2507";
...
// Run this function from the Apps Script editor to force the authorization prompt
function forceAuth() {
